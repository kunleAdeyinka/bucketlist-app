...
class BlogPostTests(BaseTestCase):
    ...
    def test_user_can_post(self):
        with self.client:
            ...
            response = self.client.post('/', data=dict(title="test", description="test"), follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'New post was successfully posted. Thanks.', response.data)
